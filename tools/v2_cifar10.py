...
setup_logging_color_message_only(file_name="train_v2_cifar10.log")
...
# with open('datasets/cifar0.1.txt', 'r') as file:
#     content = file.read()
...
with wandb.init(project=project_name, config=cfg_dict, name=exp_name) as run:
    for iter in range(epochs):
        ...
        logger.info("------------------------------------------------------------")
        logger.info("------ Fed Server: Federation process at Client-Side -------")
        logger.info("------------------------------------------------------------")
